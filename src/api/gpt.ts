...
export const getPersonalizedRecommendationsByGPT = async (preferences: {
  foodTypes?: string[];
  activities?: string[];
}): Promise<string[]> => {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert at providing personalized recommendations based on user preferences.",
        },
        {
          role: "user",
          content: `Based on these user preferences:
            Food Types: ${preferences.foodTypes?.join(", ") || "None selected"}
            Activities: ${preferences.activities?.join(", ") || "None selected"}
            
            Generate 3 personalized recommendations for activities or places they might enjoy. Return the response as a JSON array of strings.`,
        },
      ],
      temperature: 0.7,
    });
